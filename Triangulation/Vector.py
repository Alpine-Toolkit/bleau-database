...
class Vector(VectorInt):

    __data_type__ = np.double

    ##############################################

    @staticmethod
    def from_polar_coordinate(angle, distance=1):

        """ Create the vector distance*(cos(angle), sin(angle)).  The *angle* is in degree. """

        radians = math.radians(angle)

        return Vector((math.cos(radians), math.sin(radians))) * distance
    ...
    def to_normalised(self):
        ...
        return NormalisedVector(self.v / self.magnitude())
...
class NormalisedVector(Vector):
    ...
    def __init__(self, *args):
        ...
        super(NormalisedVector, self).__init__(*args)
